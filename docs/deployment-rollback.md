...
Deploy only the tested SHA on the existing host. Preserve the existing Nginx site, TLS configuration, PostgreSQL database, and unrelated PM2 processes.
...
Verify the session endpoint, reload the saved session, inspect citations/trace in a browser, then compare `/api/health.deploymentSha` with `<tested-sha>`.
